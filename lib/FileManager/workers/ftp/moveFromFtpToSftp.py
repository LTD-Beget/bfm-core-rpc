...
class MoveFromFtpToSftp(BaseWorkerCustomer):
    def __init__(self, source, target, paths, overwrite, *args, **kwargs):
        super(MoveFromFtpToSftp, self).__init__(*args, **kwargs)
    ...
    def run(self):
        try:
            ...
            self.logger.info("MoveFromFtpToSftp process run source = %s , target = %s" % (source_path, target_path))
                ...
                try:
                    ...
                    if source_ftp.isdir(abs_path):
                        ...
                        for current, dirs, files in source_ftp.ftp.walk(abs_path):
                            ...
                            for f in files:
                                ...
                                if not target_sftp.exists(target_file):
                                    self.move_from_ftp_to_sftp(source_ftp, target_sftp, source_file, target_file)
                        ...
                        try:
                            ...
                            if not target_sftp.exists(target_file):
                                self.move_from_ftp_to_sftp(source_ftp, target_sftp, abs_path, target_file)
                            elif self.overwrite and target_sftp.exists(target_file) and not target_sftp.isdir(
                                    target_file):
                                target_sftp.remove(target_file)
                                self.move_from_ftp_to_sftp(source_ftp, target_sftp, abs_path, target_file)
