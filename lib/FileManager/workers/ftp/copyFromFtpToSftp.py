...
class CopyFromFtpToSftp(BaseWorkerCustomer):
    def __init__(self, source, target, paths, overwrite, *args, **kwargs):
        super(CopyFromFtpToSftp, self).__init__(*args, **kwargs)
                ...
                try:
                    ...
                    if source_ftp.isdir(abs_path):
                        ...
                        if not target_sftp.exists(destination):
                            target_sftp.makedirs(destination)
                        ...
                        for current, dirs, files in source_ftp.ftp.walk(abs_path):
                            ...
                            for d in dirs:
                                ...
                                if not target_sftp.exists(target_dir):
                                    target_sftp.makedirs(target_dir)
                            ...
                            for f in files:
                                ...
                                if not target_sftp.exists(target_file):
                                    self.copy_from_ftp_to_sftp(source_ftp, target_sftp, source_file, target_file)
                        ...
                        try:
                            ...
                            if not target_sftp.exists(target_file):
                                self.copy_from_ftp_to_sftp(source_ftp, target_sftp, abs_path, target_file)
                            elif self.overwrite and target_sftp.exists(target_file) and not target_sftp.isdir(
                                    target_file):
                                target_sftp.remove(target_file)
                                self.copy_from_ftp_to_sftp(source_ftp, target_sftp, abs_path, target_file)
    ...
    def get_total(self, progress_object, paths, count_dirs=True, count_files=True):
        ...
        source_sftp = self.get_ftp_connection(self.source)
        for path in paths:
            try:
                abs_path = path
                if count_dirs:
    ...
    @staticmethod
    def copy_from_ftp_to_sftp(ftp, sftp, source_path, target_path):
        """
        ...
        :return:
        """
        with ftp.open(source_path, 'rb') as source_file_obj:
            sftp.sftp.putfo(source_file_obj, target_path)
